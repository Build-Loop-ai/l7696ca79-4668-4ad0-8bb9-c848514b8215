...
serve(async (req) => {
  ...
  try {
    ...
    // Build the assistant configuration payload
    const assistantPayload = buildAssistantPayload(org, settings, systemPrompt, elevenLabsVoiceId, transcriberLanguage);

    let assistantId: string;

    if (existingAssistantId) {
      // UPDATE existing assistant
      console.log("Updating existing Vapi assistant:", existingAssistantId, "for org:", org.name);

      const response = await fetch(`https://api.vapi.ai/assistant/${existingAssistantId}`, {
        method: "PATCH",
        headers: {
      ...
      if (!response.ok) {
        // If assistant not found in Vapi (deleted externally), create a new one
        if (response.status === 404) {
          console.log("Assistant not found in Vapi, creating new one...");
          assistantId = await createNewAssistant(vapiApiKey, assistantPayload, org.name);
          ...
          throw new Error(assistant.message || "Failed to update Vapi assistant");
        }
      } else {
        console.log("Vapi assistant updated successfully:", assistant.id);
        assistantId = assistant.id;
      }
    } else {
      // CREATE new assistant
      console.log("Creating new Vapi assistant for org:", org.name);
      assistantId = await createNewAssistant(vapiApiKey, assistantPayload, org.name);
    ...
    return new Response(
      JSON.stringify({ success: true, assistantId }),
...
async function createNewAssistant(vapiApiKey: string, payload: any, orgName: string): Promise<string> {
...
function buildAssistantPayload(org: any, settings: any, systemPrompt: string, elevenLabsVoiceId: string, transcriberLanguage: string): any {
  return {
...
function mapToTranscriberLanguage(language: string): string {
  ...
  // Check if it's a base language code that matches
  const baseLanguage = language.split("-")[0];
  const matchingKey = Object.keys(languageMap).find(
    (key) => key.startsWith(baseLanguage + "-")
  );

  if (matchingKey) {
    return languageMap[matchingKey];
  }
